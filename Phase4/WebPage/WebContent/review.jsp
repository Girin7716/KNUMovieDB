...
<div>
		...
		{
			sql = "WITH AVG_TABLE AS (SELECT R.PARENTTCONST, AVG(R.RATING) AVG FROM RATING R GROUP BY PARENTTCONST) SELECT M.TCONST, TITLE, R.PARENTACCOUNT, R.Rating, AVG FROM MOVIE M, RATING R, AVG_TABLE A WHERE M.TCONST = R.PARENTTCONST AND M.Tconst = A.PARENTTCONST ORDER BY M.TITLE";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery(); 
			while (rs.next()) {
			String movie_Tconst = rs.getString(1);
			...
			out.println("<div class = \"col-sm-2 col-md-2\">"+ " <a href = \"test.jsp?tconst="+ movie_Tconst + " \"> " + " <img src = \"movie_img/" + img_name + ".jpg\"> </a></div>");
			out.println("<a href = \"test.jsp?tconst=" + movie_Tconst + " \"> " + rs.getString(2) + "</a>");
			out.println("<br>AccountID : " + rs.getString(3));
			out.println("<br>AccountRating : " + rs.getDouble(4));
			out.println("<br>AVG      : " + rs.getDouble(5));
			out.println("<br></br>");
			out.println("<br></br>");
			out.println("<br></br>");
			out.println("<br></br>");
